...
% img = imread('./lena.bmp');
% img = imread('../datas/source20_1.tif');
% img = imread('../datas/1827v.bmp');
...
[imgH_Predict] = my_Horizontal_Direction_Prediction(img, ori_QuartPixel_interp);

% Horizontal Update
ori_QuartPixel_interp = Horizontal_Sinc_interpolation(imgH_Predict);
originNew = my_Horizontal_Update(img, imgH_Predict, ori_QuartPixel_interp);
...
[imgV_Predict] = my_Horizontal_Direction_Prediction(originHorizontal_L, ori_QuartPixel_interp);
% Begin Vertical Update
ori_QuartPixel_interp = Horizontal_Sinc_interpolation(imgV_Predict);
imgVertical_L = my_Horizontal_Update(originHorizontal_L, imgV_Predict, ori_QuartPixel_interp);
...
figure;
imshow(originLH, []);
% imshow(imgVertical_L, []);

% Begin Vertical Predict     PART II, Get the HL, HH
ori_QuartPixel_interp = Horizontal_Sinc_interpolation(originHorizontal_H);
[imgV_Predict] = my_Horizontal_Direction_Prediction(originHorizontal_H, ori_QuartPixel_interp);
% Begin Vertical Update
ori_QuartPixel_interp = Horizontal_Sinc_interpolation(imgV_Predict);
imgVertical_H = my_Horizontal_Update(originHorizontal_H, imgV_Predict, ori_QuartPixel_interp);
...
imgVerticalH = my_Inverse_Horizontal_Update(imgVertical_HL, imgV_QuartPixel_interp);
...
imgVerticalL = my_Inverse_Horizontal_Update(imgVertical_LL, imgV_QuartPixel_interp);
% Begin Inverse Predict
% !!!!
% For redundant ADL-Wavelet, only need inverse Update, No need for Inverse
% Prediction.
...
imgRes = my_Inverse_Horizontal_Update(imgHorizontalL, imgH_QuartPixel_interp);
