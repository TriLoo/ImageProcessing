...
int histEqu::histEquNpp(Mat &imgOut, Mat &imgIn)
{
    ...
    int nBufferSize;
    nppiMinMaxGetBufferHostSize_8u_C1R({cols, rows}, &nBufferSize);
    ...
    nppState = nppiMinMax_8u_C1R(pSrc, pStep, {cols, rows}, pMin, pMax, pDeviceBuffer);
    ...
    nppiHistogramEvenGetBufferSize_8u_C1R({cols, rows}, levelCount, &nBufferSize);
    cudaCheckErrors(cudaMalloc((void **)&pDeviceBuffer, sizeof(Npp8u) * nBufferSize));

    // calculate the hist
    nppiHistogramEven_8u_C1R(pSrc, pStep, {cols, rows}, d_hist, levelCount, 0, binCount, pDeviceBuffer);
    ...
    cout << "pDstStep = " << pDstStep << endl;   // for test

    nppState = nppiLUT_8u_C1R(pSrc, pStep, pDst, pDstStep, {cols, rows}, d_lut, d_level, levelCount);
    assert(nppState == NPP_SUCCESS);

    // copy image back from device to host
    cudaCheckErrors(cudaMemcpy2D(imgOut.data, sizeof(uchar) * cols, pDst, pDstStep, pDstStep, rows, cudaMemcpyDeviceToHost));
