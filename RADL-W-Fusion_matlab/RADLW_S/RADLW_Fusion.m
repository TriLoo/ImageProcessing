...
% lambda = 0.001;
% cA = fuse_pca(cA_A, cA_B);          % Not good
cA = (0.5 + 0.5 * (W_B(:, :, 1) - W_B(:, :, 2))) .*  cA_A + (0.5 + 0.5 * (W_B(:, :, 2) - W_B(:, :, 1))) .* cA_B;
% cA = W_B(:, :, 1) .* cA_A + W_B(:, :, 2) .* cA_B;
% cA_t = (0.5 + 0.5 * (W_B(:, :, 1) - W_B(:, :, 2))) .*  cA_A + (0.5 + 0.5 * (W_B(:, :, 2) - W_B(:, :, 1))) .* cA_B;
% cA = Solve_Optimal(cA_t, cA_A, cA_B, lambda);
