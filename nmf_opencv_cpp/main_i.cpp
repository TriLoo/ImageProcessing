...
int main(int argc, char **argv)
{
	...
	/* -------------vim for test-----------------*/
	/* -------------vim for test-----------------*/
	Mat Temp = Mat::ones(row, col, CV_32F);
	Mat WSD = Mat::zeros(row, col, CV_32F);
	WSD = matDiagM(WS);
	Temp = US * WSD * VTS;
	Temp.convertTo(Temp, CV_8UC1, 1.0);
	...
	// scala the U & V matrix obtained by SVD::compute
	float VTSnorm = norm(VTS, NORM_L1);
	US = US * VTSnorm;
	VTS = VTS / VTSnorm;

	cout << "2.1" << endl; 				// for test
	...
	{
		//cout << "3.1" << endl; 			// for test
		H = H.mul(W.t() * (M/(W*H)));
		//cout << "3.2" << endl; 			// for test
		W = W.mul(M*(H.t())/(W*H*H.t()));
		//cout << "3.3" << endl; 			// for test
		W = W/(One * matSum(W));
	}
	cout << "4" << endl;
