...
{
    cudaCheckErrors(cudaMalloc((void**)&d_imgIn_, sizeof(float) * wid * hei));
    cudaCheckErrors(cudaMalloc((void **)&d_imgOut_, sizeof(float) * wid * hei));
}
...
// process row
// 加运算比移位运算优先级高
__host__ __device__ void d_boxfilter_x(float *Out, float *imgIn, int wid, int hei, int filterR)
{
...
// process column
__host__ __device__ void d_boxfilter_y(float *imgOut,float *imgIn, int wid, int hei, int filterR)
{
    float scale = 1.0f / (float)((filterR << 1) + 1);
    //float scale = 0.0322581;
